...
function ensureBoundedRewards(rewards: number[]) {
...
function ensurePositiveParams(alpha: number, beta: number) {
  if (!(alpha > 0)) fail(`alpha must be > 0, got ${alpha}`);
  if (!(beta > 0)) fail(`beta must be > 0, got ${beta}`);
}

function ensureDqnShape(vector: number[], expectedLength = 24) {
...
function sampleReplayBuffer<T>(buffer: T[], size: number): T[] {
