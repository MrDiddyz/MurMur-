...
WEBHOOK_URL="${WEBHOOK_URL:-http://127.0.0.1:3001/webhooks/tiktok}"
TIKTOK_WEBHOOK_SECRET="${TIKTOK_WEBHOOK_SECRET:-dev-tiktok-webhook-secret}"

docker compose -f "$COMPOSE_FILE" exec -T \
  -e WEBHOOK_URL="$WEBHOOK_URL" \
  -e TIKTOK_WEBHOOK_SECRET="$TIKTOK_WEBHOOK_SECRET" \
  billing node --input-type=module <<'NODE'
import crypto from "node:crypto";

const webhookUrl = process.env.WEBHOOK_URL ?? "http://127.0.0.1:3001/webhooks/tiktok";
const secret = process.env.TIKTOK_WEBHOOK_SECRET ?? "dev-tiktok-webhook-secret";

function signPayload(body, timestamp, key) {
  return crypto.createHmac("sha256", key).update(`${timestamp}.${body}`).digest("hex");
}

async function send(payload, signature, timestamp) {
