...
function clusterSignals(signals: Signal[]): SignalCluster[] {
  ...
  return Array.from(map.entries()).map(([theme, cluster]) => ({ theme, signals: cluster }));
}

export function buildNarratives(signals: Signal[]): Omit<Narrative, "id" | "timestamp">[] {
  return clusterSignals(signals).map(({ theme, signals: cluster }, index) => {
    ...
    const confidence = clamp(0.4 + avgWeight * 0.05 - negativeRate * 0.15, 0.1, 0.98);
